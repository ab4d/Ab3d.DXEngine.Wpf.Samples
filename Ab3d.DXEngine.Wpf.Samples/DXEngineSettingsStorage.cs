...
namespace Ab3d.DXEngine.Wpf.Samples
{
    public class DXEngineSettingsStorage : IDXEngineSettingsStorage
    {
        ...
        public string UserGraphicProfileText
        {
            get
            {
                string dxEngineGraphicProfileText = Properties.Settings.Default.DXEngineGraphicProfile;

                if (string.IsNullOrEmpty(dxEngineGraphicProfileText))
                {
                    // Maybe we have upgraded the application - try to read the settings from the previous version
                    Properties.Settings.Default.Upgrade();

                    dxEngineGraphicProfileText = Properties.Settings.Default.DXEngineGraphicProfile;
                }

                return dxEngineGraphicProfileText;
            }
            set
            {
                Properties.Settings.Default.DXEngineGraphicProfile = value;
